...
describe('wallet store, create new wallet', () => {
  ...
  it('unlockWallet, before load', () => {
    expect(() => {
      wallet.unlockWallet('12345678')
    }).not.toThrow()
    expect(wallet.unlockWallet('12345678')).toBe(false)
    expect(wallet.unlockErrTimes).toBe(0)
  })

  it('checkPassword, before load', () => {
    expect(() => {
  ...
  it('create with mnemonic', () => {
    wallet.create('12345678', 'unusual drastic patrol mansion fuel more obey acquire disagree head trip chat')
  ...
  it('unlockWallet, error password', () => {
  ...
  it('unlockWallet, wrong too many times', () => {
    _.range(8).forEach(() => {
      wallet.unlockWallet('1')
    })
    expect(wallet.showLock).toBe(true)
